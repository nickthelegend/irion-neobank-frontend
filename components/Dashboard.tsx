...
import { Wallet, Send, Repeat, PiggyBank, Landmark, Users, Power, Code, Loader2 } from "lucide-react";
...
type Action = "send" | "convert" | "earn" | "borrow" | "payroll";
...
export default function Dashboard() {
  ...
  const callsFor: Record<Action, ApiCall[]> = {
    ...
    <div className="min-h-screen bg-background text-foreground font-display">
      ...
      <header className="h-16 border-b border-white/10 flex items-center justify-between px-6">
        <div className="flex items-center gap-2">
          ...
          <span className="text-[9px] text-white/30 uppercase tracking-widest ml-1 hidden sm:inline">· on Irion</span>
        </div>
        <div className="flex items-center gap-3">
          <button onClick={() => setDev(true)} className="flex items-center gap-1.5 text-[11px] font-bold uppercase tracking-widest text-white/50 hover:text-primary transition-colors"><Code className="w-3.5 h-3.5" /> Developer</button>
          <div className="hidden sm:flex flex-col items-end leading-tight">
            <span className="text-[10px] text-white/70 font-semibold">{account?.name}</span>
            <code className="text-[9px] text-white/30">{short(account?.party)}</code>
          </div>
          <button onClick={logout} className="p-2 rounded-lg bg-white/5 border border-white/10 text-white/50 hover:text-red-400 transition-colors" title="Sign out"><Power className="w-4 h-4" /></button>
        </div>
      </header>
      ...
      <main className="max-w-4xl mx-auto px-6 py-8">
        ...
        <div className="flex gap-2 mb-4 overflow-x-auto">
          {TABS.map(({ id, label: l, icon: Icon }) => (
            <button key={id} onClick={() => { setAction(id); setMsg(null); }}
              className={`flex items-center gap-2 px-4 py-2.5 rounded-xl text-xs font-bold whitespace-nowrap transition-colors ${action === id ? "bg-primary text-black" : "bg-white/5 text-white/60 hover:text-white"}`}>
              <Icon className="w-3.5 h-3.5" /> {l}
            </button>
          ))}
        </div>

        {msg && <div className={`mb-4 text-xs rounded-xl px-4 py-3 border ${msg.kind === "ok" ? "text-primary border-primary/30 bg-primary/5" : "text-red-400 border-red-500/30 bg-red-500/5"}`}>{msg.text}</div>}

        {/* action panel */}
        <div className="bg-white/[0.03] border border-white/10 rounded-2xl p-6 mb-8">
          {action === "send" && (
            <div className="flex flex-col gap-3 max-w-md">
              <h3 className="font-bold text-sm mb-1">Send money</h3>
              ...
              <button className={btn} disabled={busy || !to || !sendAmt} onClick={() => run(async () => { const r = await nb.transfer(to.trim(), Number(sendAmt), sendCur); setTo(""); setSendAmt(""); return r; }, `Sent ${sendAmt} ${sendCur}`)}>{busy ? <Loader2 className="w-4 h-4 animate-spin" /> : <Send className="w-4 h-4" />} Send</button>
            </div>
          )}

          {action === "convert" && (
            <div className="flex flex-col gap-3 max-w-md">
              <h3 className="font-bold text-sm mb-1">Convert currency (FX)</h3>
              ...
              <button className={btn} disabled={busy || !fxAmt || !fxRate || fxFrom === fxTo} onClick={() => run(async () => { const r = await nb.rebalance(fxFrom, fxTo, Number(fxAmt)); setFxAmt(""); return r; }, `Converted ${fxAmt} ${fxFrom} → ${fxTo}`)}>{busy ? <Loader2 className="w-4 h-4 animate-spin" /> : <Repeat className="w-4 h-4" />} Convert</button>
            </div>
          )}

          {action === "earn" && (
            <div className="flex flex-col gap-3 max-w-md">
              <h3 className="font-bold text-sm mb-1">Earn yield</h3>
              ...
              <div className="flex gap-2">
                <button className={btn + " flex-1"} disabled={busy || !earnAmt} onClick={() => run(async () => { const r = await nb.sweep(Number(earnAmt)); setEarnAmt(""); return r; }, `Moved ${earnAmt} to savings`)}>{busy ? <Loader2 className="w-4 h-4 animate-spin" /> : <PiggyBank className="w-4 h-4" />} Move to savings</button>
                <button className="bg-white/5 border border-white/10 px-4 py-2.5 rounded-xl font-bold text-[11px] uppercase tracking-widest text-white/70 hover:text-white transition-all disabled:opacity-50" disabled={busy || !(t?.yieldValue > 0)} onClick={() => run(() => nb.redeem(), "Redeemed savings to cash")}>Withdraw all</button>
              </div>
            </div>
          )}
          ...
          {action === "borrow" && (
            <div className="flex flex-col gap-3 max-w-md">
              <h3 className="font-bold text-sm mb-1">Borrow working capital</h3>
              {credit ? (
                <p className="text-[11px] text-white/40">Credit line <span className="text-white/80 font-bold">${fmt(credit.available)}</span> available · score {credit.score}</p>
              ) : (
                <p className="text-[11px] text-white/40">No credit line yet — get assessed first.</p>
              )}
              <div className="flex gap-2">
                <button className="bg-white/5 border border-white/10 px-4 py-2.5 rounded-xl font-bold text-[11px] uppercase tracking-widest text-white/70 hover:text-white transition-all disabled:opacity-50" disabled={busy} onClick={() => run(() => nb.underwrite(), "Credit line assessed")}>{busy ? <Loader2 className="w-4 h-4 animate-spin" /> : null} {credit ? "Re-assess" : "Get a credit line"}</button>
              </div>
              ...
              <button className={btn} disabled={busy || !borrowAmt || !(credit?.available > 0)} onClick={() => run(async () => { const r = await nb.drawLoan(Number(borrowAmt)); setBorrowAmt(""); return r; }, `Drew ${borrowAmt} USDC`)}>{busy ? <Loader2 className="w-4 h-4 animate-spin" /> : <Landmark className="w-4 h-4" />} Draw</button>
              {loans.length > 0 && (
          ...
          {action === "payroll" && (
            <div className="flex flex-col gap-4 max-w-lg">
              <div>
                <h3 className="font-bold text-sm mb-1">Pay your team — privately</h3>
              ...
              {employees.length > 0 && (
                <div className="flex flex-col gap-2">
                  ...
                  <button className={btn} disabled={busy || !Object.values(sel).some(Boolean)} onClick={() => run(async () => { const ids = Object.keys(sel).filter((k) => sel[k]); const r = await nb.runPayroll(ids.map((employeeId) => ({ employeeId }))); setSel({}); return r; }, "Payroll run — salaries paid privately")}>{busy ? <Loader2 className="w-4 h-4 animate-spin" /> : <Users className="w-4 h-4" />} Run payroll for selected</button>
                </div>
              )}
              <div className="border-t border-white/5 pt-3 flex flex-col gap-2">
                ...
                <button className="bg-white/5 border border-white/10 px-4 py-2.5 rounded-xl font-bold text-[11px] uppercase tracking-widest text-white/70 hover:text-white transition-all disabled:opacity-50 self-start" disabled={busy || !empName || !empSalary} onClick={() => run(async () => { const r = await nb.addEmployee({ name: empName.trim(), salary: Number(empSalary) }); setEmpName(""); setEmpSalary(""); return r; }, "Employee added")}>+ Add</button>
              </div>
            </div>
          )}
        </div>
      ...
      <DevDrawer open={dev} onClose={() => setDev(false)} title={TABS.find((x) => x.id === action)?.label || "This screen"} calls={callsFor[action]} />
    </div>
  );
}
